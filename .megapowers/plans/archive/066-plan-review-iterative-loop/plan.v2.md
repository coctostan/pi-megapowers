...
**Step 3 — Write minimal implementation**
Add an explicit delimiter check at the start of `parseFrontmatterEntity`, before calling `matter(markdown)`:
```typescript
...
**Step 3 — Write minimal implementation**
Replace the generic validation error with per-issue mapping from zod:
```typescript
  if (!validation.success) {
    return {
      success: false,
...
Expected: FAIL — `serializeEntity(...)` does not throw yet, so the test fails with something like “expected function to throw”.
**Step 3 — Write minimal implementation**
...
**Step 3 — Write minimal implementation**
Add duplicate ID detection to `listPlanTasks` in `extensions/megapowers/plan-store.ts`:
```typescript
