...
- **v1 review:** Adopted Path A — add real `phase_back` support to runtime + tool schema, then update prompt templates.
- **v2 review:** Fixed all 6 issues:
  1. Task 1 Step 2 made deterministic (single expected failure)
  2. Task 4 dependency corrected to `[depends: 1, 2, 3]`
  3. Task 7 assertion tightened to `/mega on|off`
  4. Task 8 split into 8a (AC8) and 8b (AC9)
  5. Task 11 split into 11a (AC12) and 11b (AC13)
  6. Task 14 split into 14a (AC16 needs-fixes) and 14b (AC16 needs-rework)
- **v3 review:** Fixed 4 issues:
  1. Task 5 dependency corrected to `[depends: 2, 3]` (needs `parts`-based `buildIdlePrompt` from Task 3)
  2. Task 6 dependency corrected to `[depends: 2, 3]` (same reason)
  3. Task 8b folded into Task 8a — regression guard test added alongside roadmap test (same implementation change, two assertions)
  4. Task 14b dependency corrected to `[depends: 9, 14a]` (Step 2 verifies both sections)
...
| 8, 9 | 8a | Idle dashboard includes roadmap/milestones line + active-issue regression guard |
| 10 | 9, 10 | `phase_back` is implemented and documented in protocol prompt |
| 11 | 10 | Protocol prompt lists `learnings` artifact phase |
| 12 | 11a | `review-plan.md` numbering fixed (`### 6.`) |
| 13 | 11b | `review-plan.md` After Review references `phase_back` |
| 14 | 12 | `implement-task.md` Execution Mode tightened |
| 15 | 13 | `verify.md` references `phase_back` (not `/phase ...`) |
| 16 | 14a, 14b | `code-review.md` references `phase_back` in needs-fixes + needs-rework |
...
Expected: FAIL — `expect(received).toBe(expected) // Expected: "M2" Received: undefined` because `milestone` does not exist on the parsed issue object yet.
...
### Task 4: Idle prompt includes open issues with id/title/milestone/priority [depends: 1, 2, 3]
...
### Task 5: Idle prompt includes slash commands [depends: 2, 3]
...
### Task 6: Idle prompt includes roadmap/milestones reference [depends: 2, 3]
...
### Task 7: Dashboard idle mode includes /triage and /mega on|off hints
...
### Task 8a: Dashboard idle mode includes roadmap/milestones reference + regression guard [depends: 7]
...
it("active issue dashboard does not include idle hints (AC9)", () => {
  ...
  expect(joined).not.toContain("/mega on|off");
...
Expected: FAIL — no roadmap line in idle dashboard yet. The regression guard test (AC9) should pass immediately since the active-issue path is unchanged.
...
Expected: PASS (both AC8 and AC9 tests)
...
### Task 11a: Fix `review-plan.md` duplicate section numbering [no-test]

**Covers:** AC12

**Justification:** Prompt-template text correction — numbering fix only.
...
Expected: sequential headings (1, 2, 3, 4, 5, 6) with no duplicate `### 5.`.
...
### Task 11b: Add `phase_back` reference to `review-plan.md` After Review section [depends: 9] [no-test]

**Covers:** AC13

**Justification:** Prompt-template text correction — adding signal reference.
...
In `## After Review`, replace final sentence with:
...
### Task 14a: Update `code-review.md` needs-fixes section to use `phase_back` [depends: 9] [no-test]

**Covers:** AC16 (needs-fixes)
...
In `### If **needs-fixes**`, add explicit transition guidance:
...
Run: `rg "phase_back" prompts/code-review.md`

Expected: at least one `phase_back` match in the needs-fixes section.
...
### Task 14b: Update `code-review.md` needs-rework section to use `phase_back` [depends: 9, 14a] [no-test]

**Covers:** AC16 (needs-rework)
...
In `### If **needs-rework**`, replace stale `/phase implement` / `/phase plan` reference with:
