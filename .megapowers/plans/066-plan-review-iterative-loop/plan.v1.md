...
| 19 (writePlanTask doesn't modify others) | 18 |
...
**Step 2 — Verify**
Run: `bun run test`
Expected: All existing tests still pass. `gray-matter` and `zod` appear in `package.json` dependencies.
...
export type ParseResult<T> =
  ...
  | { success: false; errors: ParseErrorItem[] };
...
export function parseFrontmatterEntity<T>(
  ...
  if (!markdown.trimStart().startsWith("---")) {
    return {
      success: false,
      errors: [{ type: "missing_frontmatter" }],
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      errors: [{ type: "yaml", message: err?.message }],
    };
  }

  const validation = schema.safeParse(parsed.data);
  if (!validation.success) {
    const errors: ParseErrorItem[] = validation.error.issues.map((issue) => ({
...
Expected: FAIL — gray-matter may not throw on all malformed YAML. The test will reveal whether the current implementation handles this case or needs adjustment.

**Step 3 — Write minimal implementation**
If gray-matter doesn't throw on malformed YAML (it sometimes silently parses), update `parseFrontmatterEntity` to detect malformed YAML via the zod validation step (which will catch missing/wrong-typed fields). If gray-matter does throw, the existing try/catch already handles it. Adjust the YAML detection:

In `entity-parser.ts`, after the `matter()` call, add a check for when gray-matter parses but the data is empty/unexpected due to YAML issues:
```typescript
  // After matter() call, if gray-matter silently consumed malformed YAML,
  // the zod validation step will catch it as a validation error.
  // For truly malformed YAML that gray-matter throws on, the catch block handles it.
```

No code change may be needed — the test may pass as-is if gray-matter throws, or the validation errors from zod will catch it. Run the test first and adjust.
...
### Task 4: parseFrontmatterEntity — missing frontmatter error [depends: 2]
...
Expected: PASS (already implemented in Task 2). If it already passes, this task validates the behavior exists. Move on.

**Step 3 — Write minimal implementation**
Already implemented in Task 2. No changes needed.
...
### Task 5: parseFrontmatterEntity — validation error with field path [depends: 2]
...
Body`;
    ...
    // title is 123 (number) but schema expects string — OR zod coerces it.
    // count is missing, so there should be a "count" error.
    const countError = result.errors.find((e) => e.field === "count");
    expect(countError).toBeDefined();
    expect(countError!.type).toBe("validation");
    expect(countError!.message).toBeDefined();
...
Expected: PASS (already implemented in Task 2). This task validates the existing field/message behavior.
...
Expected: FAIL — `serializeEntity` is not exported / does not exist
...
export function serializeEntity<T>(
  ...
  if (!validation.success) {
    throw new Error(
      `Invalid entity data: ${validation.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join(", ")}`,
    );
  }
...
Expected: PASS (already implemented in Task 6 — the throw is in the implementation).
...
```typescript
  ...
  it("rejects missing required fields", () => {
    ...
    expect(result.success).toBe(false); // missing title
  });
```

**Step 2 — Run test, verify it fails**
Run: `bun test tests/plan-schemas.test.ts -t "rejects"`
Expected: PASS (already handled by zod enum). Validates behavior.
...
### Task 10: PlanSummarySchema [depends: 1]
...
### Task 11: PlanReviewSchema [depends: 1]
...
### Task 12: writePlanTask + file path convention [depends: 2, 8]
...
import { join } from "node:path";
import { parseFrontmatterEntity, serializeEntity, type ParseResult } from "./entity-parser.js";
...
Expected: PASS (already implemented in Task 14). This validates the duplicate detection.

**Step 3 — Write minimal implementation**
Already implemented in Task 14.
...
### Task 16: writePlanSummary + readPlanSummary [depends: 2, 10]
...
### Task 17: writePlanReview + readPlanReview [depends: 2, 11]
...
### Task 18: writePlanTask does not modify other task files [depends: 12]
...
**Step 1 — Write the failing test**
Add to the `writePlanTask` describe block:
```typescript
  it("does not modify other existing task files", () => {
    ...
    // Read task-01 content before writing task-02
...
**Step 2 — Run test, verify it fails**
Run: `bun test tests/plan-store.test.ts -t "does not modify other"`
Expected: PASS (by design, since writePlanTask only writes one file). This validates the behavior.

**Step 3 — Write minimal implementation**
Already implemented — writePlanTask writes a single file.
