...
**Revision note:** This task explicitly **(a)** passes initial context (plan/spec/learnings) to the implementer on cycle 0 (AC10/11), **(b)** returns accumulated log entries in paused results (AC7), and **(c)** returns verifier test output in completed results (AC8).
...
> **Design note:** The pipeline tool intentionally restricts dispatch to the *current* task (`state.currentTaskIndex`) only. This is a safety guardrail — running ahead of the current task would bypass TDD ordering and state machine coordination. The parent LLM can advance the task index normally before calling pipeline for a subsequent task.
...
export async function handlePipelineTool(
  ...
  if (input.taskIndex !== currentTask.index) {
    return { error: `Task ${input.taskIndex} is not the current task (${currentTask.index}). Switch tasks before running pipeline.` };
  }
