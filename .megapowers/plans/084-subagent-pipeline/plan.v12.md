...
> **Design note — current-task-only restriction (AC25 clarification):** The pipeline tool intentionally restricts dispatch to the *current* task (`state.currentTaskIndex`) only. This is a deliberate safety guardrail — running ahead of the current task would bypass TDD ordering and state machine coordination. The parent LLM advances the task index normally (via `task_done`) before calling pipeline for a subsequent task. AC25 says "the specified plan task" and AC26 says "task index exists + deps satisfied"; the current-task restriction is a *stricter* enforcement that satisfies both ACs while preventing out-of-order execution.
