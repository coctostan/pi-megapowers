...
> **v4 revision notes:** Addresses review feedback: (1) Task 16 implementer prompt now explicitly calls `megapowers_signal({ action: "tests_failed" })` after the expected failing test run to unlock production writes when satellite enforcement is active; (2) plan narrative now explicitly documents that v1 retains the current satellite hard gate for write-ordering while still using auditor + reviewer as the soft compliance gate; (3) Task 19 Step 2 now names a deterministic pre-deletion import (`subagent-runner.js`) expected to fail the test before cleanup.
