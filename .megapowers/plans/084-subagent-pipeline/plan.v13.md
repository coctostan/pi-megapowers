...
describe("runPipeline", () => {
  it("happy path: implement -> verify(pass) -> review(approve) => completed (includes test output)", async () => {
    ...
    // AC10/11: implementer receives plan/spec context on cycle 0
  ...
  it("verify failure retries implement->verify, passing failure output into retry context (AC5)", async () => {
    ...
    // AC5: verify failure output (from tool_result, not assistant text) is included in second implementer's context
  ...
  it("review rejection retries full implement->verify->review with findings in context (AC6)", async () => {
    ...
    // AC6: second-cycle implementer context must contain the review findings from cycle 0
...
tools: read, write, edit, bash, grep, find, ls, megapowers_signal
thinking: low
---
...
3. Call `megapowers_signal({ action: "tests_failed" })` — this unlocks production file writes
4. Implement minimal production code
5. Re-run the test and confirm it PASSES
6. Call `megapowers_signal({ action: "tests_passed" })`
7. Run `bun test` and confirm no regressions

**Important:** You MUST call `megapowers_signal({ action: "tests_failed" })` after step 2 or the TDD guard will block your production file writes.
