...
> Note: This plan includes a required compatibility fix for pi-subagents-spawned sessions: **satellite detection must treat `PI_SUBAGENT_DEPTH>0` as satellite** (pi-subagents sets `PI_SUBAGENT_DEPTH`, not `PI_SUBAGENT=1`), and satellite mode must no longer apply the legacy **write-blocking TDD guard** (this issue moves TDD enforcement to prompt + deterministic audit as a soft gate). Implemented in Task 17.
...
describe("pipeline context", () => {
  ...
  it("accumulates review findings across multiple retries (AC12)", () => {
    ...
    const prompt = renderContextPrompt(ctx);
    // Both findings must be present — not just the latest
...
export interface PipelineContext {
  ...
  retryReason?: string;
  /** Accumulated across retries — each entry is one cycle's findings. */
...
export function renderContextPrompt(ctx: PipelineContext): string {
  ...
  if (ctx.accumulatedReviewFindings.length > 0) {
    sections.push(`## Accumulated Review Findings\n\n${ctx.accumulatedReviewFindings.map((f, i) => `### Cycle ${i}\n${f}`).join("\n\n")}`);
  }
...
## Task 10: Result parser (`parseStepResult`, `parseReviewVerdict`) — derives from messages [depends: 3]
...
**Revision note:** This task explicitly **(a)** passes initial context (plan/spec/learnings) to the implementer on cycle 0 (AC10/11), **(b)** returns accumulated log entries in paused results (AC7), **(c)** returns verifier test output in completed results (AC8), **(d)** adds `extractTestOutput` to `message-utils.ts` to reliably capture raw test command output from `tool_result` blocks (not just assistant text), and propagates that output into retry context for verify failures (AC5) and review rejections (AC6).
...
describe("runPipeline", () => {
  ...
  it("verify failure retries implement->verify, passing failure output into retry context (AC5)", async () => {
    ...
    const dispatcher: Dispatcher = {
      async dispatch(cfg) {
        ...
        if (cfg.agent === "verifier") {
          // Verifier only returns tool_result (no assistant text) to test robust propagation
          return mkDispatch(0, {
            messages: [
              { role: "assistant" as const, content: [{ type: "tool_use" as const, id: "t", name: "bash", input: { command: "bun test" } }] },
              {
  ...
  it("review rejection retries full implement->verify->review with findings in context (AC6)", async () => {
              ...
              messages: [
                {
                  role: "assistant" as const,
                  content: [
                    { type: "text" as const, text: "Verdict: reject\n\n## Findings\n- Missing error handling in parser\n- No edge case coverage" },
                  ],
                },
              ] as any,
            });
    ...
    expect(r.status).toBe("completed");
    expect(r.reviewVerdict).toBe("approve");
    // AC6: full retry — dispatch order must be implement, verify, review, implement, verify, review
...
### 3a) Add `extractTestOutput` to `message-utils.ts`

Modify `extensions/megapowers/subagent/message-utils.ts` — add the following export after `extractToolCalls`:

```ts
/**
 * Extract raw test command output from tool_result blocks.
 * Unlike extractFinalOutput (which reads assistant text blocks), this reads
 * the actual tool_result content from bash test commands — reliable even when
 * the verifier only returns tool_result output with no assistant text.
 */
...
export async function runPipeline(
  ...
  for (let cycle = 0; cycle <= maxRetries; cycle++) {
    ...
    if (!verifyParsed.testsPassed) {
      ...
      // AC5: extract raw test output from tool_result blocks (not just assistant text)
      // so that even when verifier only emits tool_result, failure details propagate
    ...
    if (verdict.verdict === "approve") {
      return {
        ...
        retryCount,
        testsPassed: true,
        testOutput: extractTestOutput(verify.messages) || verifyParsed.finalOutput,
        reviewVerdict: "approve",
        reviewFindings: verdict.findings,
      };
    }

    // rejected — AC6: re-run full implement → verify → review with findings
    retryCount++;
    if (cycle >= maxRetries) {
      const { diff } = await getWorkspaceDiff(options.workspaceCwd, options.execJJ);
      return {
...
> **Design note — current-task-only restriction (AC25 — CONFIRMED):** The pipeline tool intentionally restricts dispatch to the *current* task (`state.currentTaskIndex`) only. This is a deliberate safety guardrail — running ahead of the current task would bypass TDD ordering and state machine coordination. The parent LLM advances the task index normally (via `task_done`) before calling pipeline for a subsequent task. AC25 says "the specified plan task" and AC26 says "task index exists + deps satisfied"; the current-task restriction is a *stricter* enforcement that satisfies both ACs while preventing out-of-order execution. **Confirmed acceptable:** AC25 does not require dispatching arbitrary task indices — restricting to the current task is the intended design.

> **AC27 compliance — guidance required on resume:** Per AC27, the resume input shape is `{ taskIndex, resume: true, guidance: string }`. The handler validates that `guidance` is a non-empty string when `resume: true` — omitting guidance on a paused pipeline returns an error. This ensures the parent LLM provides actionable direction before burning another retry budget.
...
describe("handlePipelineTool", () => {
  ...
  it("rejects resume with empty guidance string (AC27)", async () => {
    const dispatcher: Dispatcher = { async dispatch() { return { exitCode: 0, messages: [], filesChanged: [], testsPassed: null }; } };

    const r = await handlePipelineTool(tmp, { taskIndex: 1, resume: true, guidance: "  " }, dispatcher, async () => ({ code: 0, stdout: "", stderr: "" }));
    expect(r.error).toContain("guidance");
  });
  ...
  it("paused pipeline returns log + diff + errorSummary (AC27) and resume reuses workspace", async () => {
    ...
    // AC27: paused pipeline must return log + diff + errorSummary
    ...
    // Resume reuses previous workspace (no new workspace add)
...
export async function handlePipelineTool(
  ...
  // AC27: guidance is required when resuming a paused pipeline
  if (input.resume && (!input.guidance || input.guidance.trim().length === 0)) {
    return { error: "guidance is required when resuming a paused pipeline (resume: true). Provide actionable direction for the retry." };
  }
  ...
  // Current-task-only restriction (see design note above)
  if (input.taskIndex !== currentTask.index) {
    return { error: `Task ${input.taskIndex} is not the current task (${currentTask.index}). The pipeline tool only dispatches the current task. Advance the task index via task_done before calling pipeline for a subsequent task.` };
  }

  const dep = validateTaskDependencies(currentTask.index, tasks as any, state.completedTasks);
  if (!dep.valid) {
    return { error: dep.error ?? `Task ${currentTask.index} depends on incomplete tasks: ${(dep.unmetDependencies ?? []).join(", ")}` };
  }
...
## Task 15: One-shot subagent tool handler (`subagent`) — single dispatch in workspace [depends: 8]
...
## Task 16: Add pipeline agents (`implementer`, `reviewer`, `verifier`) [no-test]
...
The pipeline runner will audit your tool-call history for TDD compliance (test-first ordering + test runs) and will pass that report to the reviewer. Follow the sequence above even though writes are not hard-blocked.
...
## Task 17: Satellite compatibility for pi-subagents (`PI_SUBAGENT_DEPTH`) + project-root resolution + disable satellite write-blocking TDD guard

**Covers:** required pi-subagents compatibility (satellite detection via `PI_SUBAGENT_DEPTH`, project-root resolution) and the spec’s **TDD enforcement shift** for subagents: disable the legacy satellite write-blocking guard so TDD is enforced via prompt + deterministic audit (soft gate).
...
import { join } from "node:path";
import { tmpdir } from "node:os";
import { resolveProjectRoot, isSatelliteMode, setupSatellite } from "../extensions/megapowers/satellite.js";
...
Expected: FAIL — `PI_SUBAGENT_DEPTH` is not recognized, `resolveProjectRoot()` does not walk up to find `.megapowers/state.json`, and `setupSatellite()` still installs write-blocking hooks / registers `megapowers_signal` (legacy guard).
...
4) Replace `setupSatellite()` with a no-op (no write/edit blocking; TDD is enforced via prompt + auditor soft gate):

```ts
export function setupSatellite(_pi: ExtensionAPI): void {
  // Satellite (subagent) sessions run in isolated jj workspaces.
  // In this pipeline architecture, TDD is guided by prompts and audited after the implement step,
  // not enforced via write-blocking tool hooks.
}
```

Also remove any now-unused imports in `extensions/megapowers/satellite.ts` (e.g. `readState`, `deriveTasks`, `canWrite`, `isTestFile`, `Type`, `TddTaskState`).
...
3) Replace the existing `subagent` tool registration block with:
...
4) Add a new `pipeline` tool registration block:

```ts
pi.registerTool({
  name: "pipeline",
  label: "Pipeline",
  description: "Run the implement → verify → review pipeline for the current plan task in an isolated jj workspace. Only dispatches the current task (state.currentTaskIndex) — advance via task_done before calling for the next task. When resuming a paused pipeline, guidance is required.",
  parameters: Type.Object({
    taskIndex: Type.Number({ description: "Task index to run (must match current task)" }),
...
5) Delete the entire `subagent_status` tool registration block.
...
**Step 5 — Verify no regressions + typecheck**
Run: `bunx tsc --noEmit && bun test`

Expected: no type errors (validates pi-subagents imports resolve), all tests passing
...
Expected: FAIL — at least one old module is still importable (e.g., `subagent-async` resolves successfully).

**Step 3 — Delete old files**

Run the following to delete every old subagent implementation and its matching test:

```bash
# Old implementation files
rm -f extensions/megapowers/subagent/subagent-agents.ts
rm -f extensions/megapowers/subagent/subagent-async.ts
rm -f extensions/megapowers/subagent/subagent-context.ts
rm -f extensions/megapowers/subagent/subagent-errors.ts
rm -f extensions/megapowers/subagent/subagent-runner.ts
rm -f extensions/megapowers/subagent/subagent-status.ts
rm -f extensions/megapowers/subagent/subagent-tools.ts
rm -f extensions/megapowers/subagent/subagent-validate.ts
rm -f extensions/megapowers/subagent/subagent-workspace.ts

# Old test files
rm -f tests/subagent-agents.test.ts
rm -f tests/subagent-async.test.ts
rm -f tests/subagent-context.test.ts
rm -f tests/subagent-errors.test.ts
rm -f tests/subagent-runner.test.ts
rm -f tests/subagent-status.test.ts
rm -f tests/subagent-tools.test.ts
rm -f tests/subagent-validate.test.ts
rm -f tests/subagent-workspace.test.ts
```

After deletion, verify no remaining imports reference the deleted modules:

```bash
grep -r "subagent-agents\|subagent-async\|subagent-context\|subagent-errors\|subagent-runner\|subagent-status\|subagent-tools\|subagent-validate\|subagent-workspace" extensions/ --include="*.ts" -l
```

If any files still import the deleted modules (most likely `register-tools.ts` — already updated in Task 18), remove those import lines.

**Step 4 — Run test, verify it passes**
Run: `bun test tests/clean-slate.test.ts`

Expected: PASS — all old modules fail to import.

**Step 5 — Verify no regressions**
Run: `bun test`

Expected: all passing (old test files are gone so their suites no longer run; no remaining code references the deleted modules).