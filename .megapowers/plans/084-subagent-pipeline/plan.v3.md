...
> **v3 revision notes:** Addresses plan review feedback: (1) Task 11 — initial context always passed, retry budget uses `maxCycles` with `< maxCycles` loop, tests use temp dirs; (2) Task 14 — robust `extractTaskSection`, TDD gate bypass via `tddTaskState` write before `task_done`, resume includes previous log context; (3) Task 17 — specific expected failure output; (4) Task 18 — `bunx tsc --noEmit` verification step.
...
function extractTaskSection(planMd: string, taskIndex: number): string | undefined {
  // Split on task headers and find matching section (robust vs regex pitfalls with multiline $)
