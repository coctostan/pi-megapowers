...
> Compatibility requirement (from Goal/Context): pi-subagents-spawned sessions must be detected via `PI_SUBAGENT_DEPTH>0`, and satellite mode must **not** install legacy write-blocking TDD hooks. Implemented in **Task 18**.

> TDD enforcement shift (from Goal): pipeline completion must not be blocked by the primary session’s hard `task_done` TDD gate. Implemented in **Task 15** by explicitly setting a skipped TDD state for the completed task before invoking task completion.
...
- **Imports:** production `.ts` files import other modules via `.js` extension.
...
**Covers AC 17 (partial: files/tests/final output extraction + tool call extraction).**
...
### Task 5: TDD auditor (`auditTddCompliance`) — deterministic ordering

**Covers AC 13, 14, 15, 16 (report is produced for pipeline context).**
...
### Task 11: Message utils — add `extractTestOutput` [depends: 3]

**Covers AC 17 (test-output extraction needed for AC5/AC8).**

**Files:**
- Modify: `extensions/megapowers/subagent/message-utils.ts`
- Test: `tests/message-utils-test-output.test.ts`

**Step 1 — Write the failing test**
Create `tests/message-utils-test-output.test.ts`:
...
**Step 2 — Run test, verify it fails**
Run: `bun test tests/message-utils-test-output.test.ts`

Expected: FAIL — `extractTestOutput` is not exported

**Step 3 — Write minimal implementation**
Modify `extensions/megapowers/subagent/message-utils.ts` by adding this export (after `extractToolCalls`):

```ts
/**
 * Extract raw test command output from tool_result blocks.
 * Reads the tool_result content for bash test commands — reliable even when
 * the verifier emits no assistant text.
 *
 * Note: `Message` is already imported at the top of message-utils.ts (Task 3).
 * Do NOT add a duplicate import here.
 */
export function extractTestOutput(messages: Message[]): string | null {
  const { isTestCommand } = __internal;
  const testBashIds = new Set<string>();
  for (const msg of messages as any[]) {
    const content = msg?.content;
    if (!Array.isArray(content)) continue;
...
### Task 12: Pipeline runner (`runPipeline`) — retries, timeouts, log, diff, pause/completed [depends: 5, 6, 7, 9, 10, 11]
...
### Task 13: Dependency validator (`validateTaskDependencies`)
...
### Task 14: Pipeline resume metadata store (`pipeline-meta`)

**Covers AC 27 (resume state persistence).**
...
### Task 15: Pipeline tool handler (`pipeline`) — validation, resume, squash+task completion, paused returns log+diff+error [depends: 8, 12, 13, 14]

**Covers AC 25, 26, 27, 28.**

**Key behavior notes (explicitly tested):**
- Dispatches the plan task specified by `taskIndex` (AC25).
- Validates dependencies before dispatch (AC26).
- On `paused`, returns `{ log, diff, errorSummary }` and supports `resume: true` with required guidance (AC27).
- On `completed`, squashes workspace and **marks task done without being blocked by the primary session’s hard TDD gate** by writing a skipped TDD state for that task before completing it (AC28 + Goal).
...
### Task 16: One-shot subagent tool handler (`subagent`) — single dispatch in workspace [depends: 8, 10]
...
### Task 17: Add pipeline agents (`implementer`, `reviewer`, `verifier`) [no-test]
...
### Task 18: Satellite compatibility for pi-subagents (`PI_SUBAGENT_DEPTH`) + project-root resolution + disable satellite write-blocking hooks

**Covers Goal compatibility requirement** (pi-subagents env + disabling legacy satellite enforcement).
...
Expected: FAIL — `PI_SUBAGENT_DEPTH` is not recognized, `resolveProjectRoot()` does not walk up to find `.megapowers/state.json`, and `setupSatellite()` still installs write-blocking hooks / registers `megapowers_signal`.
...
4) Replace `setupSatellite()` with a no-op:
...
5) Remove any now-unused imports from `extensions/megapowers/satellite.ts`.
...
### Task 19: Update `/mega off` + `/mega on` tool filtering to include `pipeline` and remove `subagent_status`

**Supports AC 25 and clean-slate replacement.**
...
**Step 1 — Write the failing test**
Create `tests/commands-tools-filter.test.ts`:
...
**Step 2 — Run test, verify it fails**
Run: `bun test tests/commands-tools-filter.test.ts`

Expected: FAIL — `commands.ts` still references `subagent_status` and does not reference `pipeline`.

**Step 3 — Write minimal implementation**
Modify `extensions/megapowers/commands.ts`:

1) In `/mega off`, set the filter to hide `pipeline` and remove `subagent_status`:
...
2) In `/mega on`, restore `pipeline` and remove `subagent_status`:
...
### Task 20: Tool wiring — update `subagent` tool to one-shot mode and remove `subagent_status`

**Covers AC 29–30 and supports AC 34 (remove old wiring).**

**Files:**
- Modify: `extensions/megapowers/register-tools.ts`
- Test: `tests/tools-subagent-wiring.test.ts`

**Step 1 — Write the failing test**
Create `tests/tools-subagent-wiring.test.ts`:
...
**Step 2 — Run test, verify it fails**
Run: `bun test tests/tools-subagent-wiring.test.ts`

Expected: FAIL — old `subagent_status` wiring still exists.
...
### Task 21: Tool wiring — register `pipeline` tool [depends: 20]

**Covers AC 25 (tool registered) and supports AC 28 (completion path).**

**Files:**
- Modify: `extensions/megapowers/register-tools.ts`
- Test: `tests/tools-pipeline-wiring.test.ts`

**Step 1 — Write the failing test**
Create `tests/tools-pipeline-wiring.test.ts`:
...
**Step 2 — Run test, verify it fails**
Run: `bun test tests/tools-pipeline-wiring.test.ts`

Expected: FAIL — `pipeline` tool is not registered.
...
### Task 22: Clean slate replacement — delete old subagent implementation/tests
...
Run: `grep -r "subagent-agents\\|subagent-async\\|subagent-context\\|subagent-errors\\|subagent-runner\\|subagent-status\\|subagent-tools\\|subagent-validate\\|subagent-workspace" extensions/ --include="*.ts" -n`
