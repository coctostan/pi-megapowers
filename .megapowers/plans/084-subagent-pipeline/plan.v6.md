...
**Revision note (v3):** Fixes from plan review: **(a)** initial context always passed to implement step (cycle 0 included), **(b)** retry budget uses `maxCycles` (default 3) with `< maxCycles` loop to avoid off-by-one, **(c)** tests use a writable temp directory for `projectRoot`.

**Revision note (v5):** Added explicit test assertion for AC 16: captures the context string passed to the reviewer dispatch and verifies it contains `"TDD report"` (the TDD compliance report JSON).
...
describe("runPipeline", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "pipeline-runner-"));
  });
  ...
  it("happy path: implement -> verify(pass) -> review(approve) => completed (includes test output)", async () => {
    ...
    const r = await runPipeline(
      ...
      {
        projectRoot: tmp,
        workspaceCwd: join(tmp, ".megapowers", "subagents", "pipe", "workspace"),
        pipelineId: "pipe",
  ...
  it("TDD compliance report is included in context passed to reviewer (AC 16)", async () => {
    let reviewerContext: string | undefined;
            ...
            messages: [
              {
                role: "assistant" as const,
                content: [
                  { type: "tool_use" as const, id: "w1", name: "write", input: { path: "tests/x.test.ts" } },
                  { type: "tool_use" as const, id: "b1", name: "bash", input: { command: "bun test" } },
                  { type: "tool_use" as const, id: "w2", name: "write", input: { path: "src/x.ts" } },
                ],
              },
              {
                role: "tool" as const,
                content: [{ type: "tool_result" as const, tool_use_id: "b1", content: "1 fail" }],
              },
            ] as any,
          });
        }
    ...
    // AC 16: TDD compliance report must be present in reviewer context
    expect(reviewerContext).toBeDefined();
    expect(reviewerContext).toContain("TDD report");
    expect(reviewerContext).toContain("testWrittenFirst");
    expect(reviewerContext).toContain("testRanBeforeProduction");
    expect(reviewerContext).toContain("testRunCount");
  });
  ...
  it("verify failure retries implement->verify and pauses after budget exhausted (includes accumulated log)", async () => {
    ...
    expect(implCount).toBe(2); // maxCycles=2 → 2 attempts
...
export async function runPipeline(
  ...
  for (let cycle = 0; cycle < maxCycles; cycle++) {
    ...
    ctx = setRetryContext(ctx, `Review rejected`, verdict.findings);
  }
...
**Revision note (v3):** Fixes from plan review: **(a)** `extractTaskSection` uses robust boundary-based parsing instead of fragile regex, **(b)** sets `tddTaskState` to `{ state: "impl-allowed", taskIndex, skipped: false }` before calling `handleSignal("task_done")` to bypass the hard TDD gate (aligning with spec's shift to audit+soft-gate model), **(c)** on resume, reads previous pipeline log and includes last error summary in the initial context.
...
import { readPipelineLog } from "./pipeline-log.js";
...
function extractTaskSection(planMd: string, taskIndex: number): string | undefined {
  const sections = planMd.split(/(?=^###\s+Task\s+\d+:)/m);
  const match = sections.find((s) => new RegExp(`^###\\s+Task\\s+${taskIndex}:`).test(s));
  return match?.trim();
}
...
export async function handlePipelineTool(
  ...
  let taskDescription = currentTask.description;

  // On resume, include previous log context (AC12/AC27)
  if (input.resume) {
    const prevLog = readPipelineLog(projectRoot, pipelineId);
  ...
  if (result.status === "completed") {
    ...
    // Bypass hard TDD gate: pipeline uses audit+soft-gate model, so we
    // pre-set tddTaskState to satisfy the existing task_done check.
    const preState = readState(projectRoot);
    writeState(projectRoot, {
      ...preState,
      tddTaskState: { state: "impl-allowed", taskIndex: currentTask.index, skipped: false },
    });
...
## Task 15: One-shot subagent tool handler (`subagent`) — single dispatch in workspace [depends: 8, 10]
...
3. After confirming the failing run, call `megapowers_signal({ action: "tests_failed" })` to unlock production writes in satellite mode when enforcement is active.
4. Implement minimal production code
5. Re-run the test and confirm it PASSES
6. Run `bun test` and confirm no regressions

This v1 plan keeps the current satellite hard write gate for implementation ordering, while the pipeline reviewer still receives the deterministic TDD auditor report as a soft compliance signal.
```
...
**Covers:** required compatibility (not explicitly in AC list, but required for pipeline to run in subagent-spawned sessions).
...
Expected: FAIL — `isSatelliteMode({ isTTY: false, env: { PI_SUBAGENT_DEPTH: "1" } })` returns `false` (expected `true`), and `resolveProjectRoot(cwd, {})` returns `cwd` instead of `root` because it doesn't walk up to find `.megapowers/state.json`.
...
## Task 18: Commands filtering — update `/mega off` and `/mega on` tool lists

**Covers:** supports AC 25 (pipeline tool visibility in mega on/off).
...
**Step 1 — Write the failing test**
Create `tests/commands-filtering.test.ts`:

```ts
import { describe, it, expect } from "bun:test";

describe("commands filtering", () => {
  it("/mega off filtering mentions pipeline tool and excludes subagent_status", async () => {
...
Expected: FAIL — commands.ts still references `"subagent_status"` and does not contain `"pipeline"`.
...
## Task 19: Tool registration wiring — register `pipeline`, rewrite `subagent`, remove `subagent_status` [depends: 4, 14, 15]
...
Expected: FAIL — register-tools.ts does not include pipeline tool and still contains subagent_status.
...
3) Add a new `pipeline` tool registration block:
...
**Step 5 — Verify no regressions and typecheck pi-subagents imports**
Run: `bun test`
...
Expected: no type errors. This validates that all `pi-subagents` imports (`runSync`, `discoverAgents`, `AgentConfig`, `RunSyncOptions`, `SingleResult`) resolve correctly against the actual package exports.

---

## Task 20: Clean slate replacement — delete old subagent implementation/tests [depends: 19]
...
describe("clean slate subagent replacement", () => {
  it("all old subagent modules are not importable", async () => {
    const oldMods = [
      "../extensions/megapowers/subagent/subagent-agents.js",
      "../extensions/megapowers/subagent/subagent-async.js",
      "../extensions/megapowers/subagent/subagent-context.js",
      "../extensions/megapowers/subagent/subagent-errors.js",
      ...
      "../extensions/megapowers/subagent/subagent-workspace.js",
    ];
...
Expected: FAIL — importing `../extensions/megapowers/subagent/subagent-runner.js` still succeeds before deletion, so the first `expect(threw).toBe(true)` assertion fails deterministically.
