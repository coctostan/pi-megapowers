# Plan v2: Generalize State Machine (#071)
...
Expected: Same pass/fail count as baseline (574 pass, 3 pre-existing fail)
...
export const featureWorkflow: WorkflowConfig = {
  name: "feature",
  phases: [
    { name: "brainstorm", openEnded: true, blocking: true, promptTemplate: "brainstorm.md", guidance: "Send a message to brainstorm your idea." },
...
export const bugfixWorkflow: WorkflowConfig = {
  name: "bugfix",
  phases: [
    { name: "reproduce", artifact: "reproduce.md", openEnded: true, blocking: true, promptTemplate: "reproduce-bug.md", guidance: "Send a message to reproduce the bug." },
    { name: "diagnose", artifact: "diagnosis.md", openEnded: true, blocking: true, promptTemplate: "diagnose-bug.md", guidance: "Send a message to diagnose the root cause." },
    { name: "plan", artifact: "plan.md", blocking: true, promptTemplate: "write-plan.md", guidance: "Send a message to generate the plan." },
    { name: "review", needsReviewApproval: true, blocking: true, promptTemplate: "review-plan.md", guidance: "Send a message to review the plan." },
    { name: "implement", tdd: true, promptTemplate: "implement-task.md" },
    { name: "verify", artifact: "verify.md", blocking: true, promptTemplate: "verify.md", guidance: "Send a message to verify the implementation." },
...
describe("deriveToolInstructions", () => {
  ...
  it("returns phase_next for brainstorm (no artifact, open-ended)", () => {
    const phase = featureWorkflow.phases.find(p => p.name === "brainstorm")!;
    const instructions = deriveToolInstructions(phase);
    expect(instructions).toContain("phase_next");
    expect(instructions).not.toContain("save_artifact");
  });
 ...
 */
export function deriveToolInstructions(phase: PhaseConfig): string {
  const parts: string[] = [];
  ...
  // Artifact phase: save then advance (covers spec, plan, verify, code-review, reproduce, diagnose)
  if (phase.artifact) {
  ...
  // Default: just advance (brainstorm, done)
  parts.push(
...
### Task 14: Refactor write-policy.ts to use workflow config [depends: 10]

**Files:**
- Modify: `extensions/megapowers/policy/write-policy.ts`
- Test: `tests/tool-overrides.test.ts` (existing — no changes, acts as regression)

**Step 1 — Write the failing test**

No new test — existing `tests/tool-overrides.test.ts` validates write policy behavior.
...
Replace the hardcoded `artifactMap` block and bugfix aliasing block with config-driven artifact loading:
...
Replace the `PHASE_TOOL_INSTRUCTIONS` lookup with config-driven derivation:

Remove the `PHASE_TOOL_INSTRUCTIONS` constant entirely.
...
```ts
  ...
  if (state.workflow && state.phase) {
    ...
    if (phaseConfig) {
      const toolInstructions = deriveToolInstructions(phaseConfig);
      if (toolInstructions) parts.push(toolInstructions.trim());
    }
  }
```

Also handle the `done` phase tool instructions — since `done` has no artifact/tdd/review flags, `deriveToolInstructions` returns the default "phase_next" message. The existing `done` entry in `PHASE_TOOL_INSTRUCTIONS` says to use `megapowers_save_artifact` for done-phase outputs. Add this to the done `PhaseConfig` guidance or handle it in the done-mode template section (which already exists and is unaffected by this change). The done phase prompt is handled by the `doneMode` template block already, so this is fine.
...
Run: `cd /Users/maxwellnewman/pi/workspace/pi-megapowers && bun test`
Expected: 574 pass, 3 pre-existing fail (same baseline as before refactor)

**Step 2 — Verify no new failures**
Compare output to baseline. Any new failure indicates a behavioral regression from the refactor.
