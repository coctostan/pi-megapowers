...
**Files:**
- Modify: `extensions/megapowers/tools/tool-plan-task.ts` (already implemented in Task 7)
- Test: `tests/tool-plan-task.test.ts`
...
describe("handlePlanTask — update (partial merge)", () => {
  ...
  it("works in revise mode", () => {
    setState(tmp, { phase: "plan", planMode: "revise", planIteration: 2 });
    // Pre-create task file
    handlePlanTask(tmp, { id: 1, title: "T", description: "B." }); // works in revise for new
    // But actually, revise mode should also allow creating new tasks? Let me re-check the spec.
    // Actually spec says revise mode can update tasks, and plan_task can be called in revise mode.
...
**Step 2 — Run test, verify it fails**
Run: `bun test tests/tool-plan-task.test.ts`
Expected: FAIL — The update tests should fail if the partial merge logic has bugs. Since the implementation was included in Task 7, these tests should pass. If they don't, debug the merge logic.

**Step 3 — Write minimal implementation**

The `handleUpdate` function was already implemented in Task 7. If tests fail, fix the merge logic in the `handleUpdate` function. The key behaviors:
- Only overwrite fields that are explicitly provided (`!== undefined`)
- Preserve `existing.content` when `params.description` is undefined
- Track changed fields for the response message
...
### Task 15: Prompt routing — planMode-aware template selection [depends: 4]
...
### Task 16: Prompt templates — update write-plan.md, review-plan.md, create revise-plan.md [no-test]
...
**`prompts/write-plan.md`** — Add instructions to use `megapowers_plan_task` tool:

Add near the end of the existing template (before "When the plan is complete"):
```markdown
## Saving Tasks
...
**`prompts/review-plan.md`** — Replace the approval instructions:

Replace the existing "## What to do" section with:
```markdown
## What to do

1. Read all task files in `.megapowers/plans/{{issue_slug}}/tasks/` directory
2. Evaluate each task against the criteria below
3. Submit your verdict via the `megapowers_plan_review` tool:
...
**For frontmatter changes** (dependencies, no_test, files):
```
...
When all revisions are complete, call `megapowers_signal({ action: "plan_draft_done" })` to resubmit for review.
```
...
Covers AC 32, 33, 34 (prompt templates for each mode).
...
```typescript
// Add to existing tests/register-tools.test.ts (or create if it doesn't exist):
import { describe, it, expect, beforeEach, afterEach } from "bun:test";
import { mkdtempSync, rmSync, mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { tmpdir } from "node:os";
...
describe("full plan loop integration", () => {
  let tmp: string;

  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "plan-loop-integration-"));
  });
  ...
  it("draft → review → approve → implement loop works end-to-end", () => {
    // Start in plan phase with draft mode
    setState(tmp, { phase: "plan", planMode: "draft", planIteration: 1 });

    // Create tasks via tool
    const r1 = handlePlanTask(tmp, { id: 1, title: "First", description: "Body 1." });
    expect(r1.error).toBeUndefined();
    const r2 = handlePlanTask(tmp, { id: 2, title: "Second", description: "Body 2.", depends_on: [1] });
    expect(r2.error).toBeUndefined();

    // Signal draft done → transitions to review
    const draftDone = handleSignal(tmp, "plan_draft_done");
    expect(draftDone.error).toBeUndefined();
    expect(readState(tmp).planMode).toBe("review");

    // Approve via review tool → transitions to implement
    const approve = handlePlanReview(tmp, {
      verdict: "approve",
      feedback: "Looks good.",
      approved_tasks: [1, 2],
    });
    ...
    expect(finalState.phase).toBe("implement");
    expect(finalState.planMode).toBeNull();
  });

  it("draft → review → revise → review loop works", () => {
    setState(tmp, { phase: "plan", planMode: "draft", planIteration: 1 });

    handlePlanTask(tmp, { id: 1, title: "T1", description: "B1." });
    handleSignal(tmp, "plan_draft_done");

    // Revise
    const revise = handlePlanReview(tmp, {
      verdict: "revise",
      feedback: "Fix task 1.",
      approved_tasks: [],
      needs_revision_tasks: [1],
    ...
    // Update task in revise mode
    const update = handlePlanTask(tmp, { id: 1, title: "T1 revised" });
    expect(update.error).toBeUndefined();

    // Signal draft done again → back to review
    const draftDone2 = handleSignal(tmp, "plan_draft_done");
    expect(draftDone2.error).toBeUndefined();
    expect(readState(tmp).planMode).toBe("review");
  });
});
```
...
Expected: FAIL if the `plan_draft_done` action isn't wired into `handleSignal`'s type union in register-tools.ts. Or PASS if all handlers are properly wired from previous tasks.
...
| 35 | 9, 12 | newSession on mode transitions |
